...
def deactive_table_dhcp():
    con = sqlite3.connect(db_name)
    print('Деактивация данных в таблице dhcp...')
    with con:
...
def del_old_data(db_name):
    ...
    for row in result:
        if str(row['last_active']) < str(week_ago):
            value = row['mac']
            con.execute('''delete from dhcp where mac = ?''', (value, ))
            con.commit()
    con.close()
